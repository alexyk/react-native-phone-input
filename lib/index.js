  ...
  updateFlagAndFormatNumber(number, actionAfterSetState = null) {
    ...
    if (number) {
      ...
      formattedPhoneNumber = allowZeroAfterCountryCode
        ...
        const dialCode = PhoneNumber.getDialCode(formattedPhoneNumber);
        const numeric = PhoneNumber.getNumeric(dialCode);
        const possibleCountries = Country.getCountryCodes()[numeric];
        const existsCode = (possibleCountries != null && possibleCountries.length > 0);
    
        if (iso2) {
          if (existsCode) {
            !possibleCountries.includes(iso2) && (iso2 = possibleCountries[0]);
          } else {
            iso2 = "";
          }
        } else {
          existsCode && (iso2 = possibleCountries[0]);
        }
      }
