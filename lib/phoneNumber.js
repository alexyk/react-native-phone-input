...
class PhoneNumber {
  ...
  getCountryCodeOfNumber(number) {
    ...
    if (countryCode) {
      return lodash.first(countryCode.filter(iso2 => iso2));
    }

    return '';
  }

  getPossibleCountryCodesOfNumber(number) {
    const dialCode = this.getDialCode(number);
    const numeric = this.getNumeric(dialCode);
    const countryCode = Country.getCountryCodes()[numeric];

    if (countryCode) {
      return countryCode.filter(iso2 => iso2);
    }
  ...
  getNumberType(number, iso2) {
    ...
    return lodash.findKey(numberType, noType => noType === type);
  }
